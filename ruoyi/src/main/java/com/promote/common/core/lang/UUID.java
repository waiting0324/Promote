package com.promote.common.core.lang;
...
import com.promote.common.exception.UtilException;
...
public final class UUID implements java.io.Serializable, Comparable<UUID>
{
    ...
    /**
     * 獲取型別 4（偽隨機生成的）UUID 的靜態工廠。 使用加密的本地執行緒偽隨機數生成器生成該 UUID。
     * 
    ...
    /**
     * 獲取型別 4（偽隨機生成的）UUID 的靜態工廠。 使用加密的強偽隨機數生成器生成該 UUID。
     * 
     * @return 隨機生成的 {@code UUID}
     */
     ...
     * 獲取型別 4（偽隨機生成的）UUID 的靜態工廠。 使用加密的強偽隨機數生成器生成該 UUID。
     * 
     * @param isSecure 是否使用{@link SecureRandom}如果是可以獲得更安全的隨機碼，否則可以得到更好的效能
     * @return 隨機生成的 {@code UUID}
     */
    ...
    /**
     * 根據指定的位元組陣列獲取型別 3（基於名稱的）UUID 的靜態工廠。
     *
     * @param name 用於構造 UUID 的位元組陣列。
     *
     * @return 根據指定陣列生成的 {@code UUID}
     */
    ...
    /**
     * 根據 {@link #toString()} 方法中描述的字串標準表示形式建立{@code UUID}。
     *
     * @param name 指定 {@code UUID} 字串
     * @return 具有指定值的 {@code UUID}
     * @throws IllegalArgumentException 如果 name 與 {@link #toString} 中描述的字串表示形式不符丟擲此異常
     *
     */
     ...
     * <ul>
     * <li>0 為 NCS 向後相容保留
     * <li>2 <a href="http://www.ietf.org/rfc/rfc4122.txt">IETF&nbsp;RFC&nbsp;4122</a>(Leach-Salz), 用於此類
     * <li>6 保留，微軟向後相容
     * <li>7 保留供以後定義使用
     * </ul>
     *
     ...
     *
     * <p>
     * 時間戳值僅在在基於時間的 UUID（其 version 型別為 1）中才有意義。<br>
     * 如果此 {@code UUID} 不是基於時間的 UUID，則此方法丟擲 UnsupportedOperationException。
     *
     ...
     * <p>
     * {@code clockSequence} 值僅在基於時間的 UUID（其 version 型別為 1）中才有意義。 如果此 UUID 不是基於時間的 UUID，則此方法丟擲
     ...
     *
     * <p>
     * 48 位的節點值根據此 UUID 的 node 欄位構造。此欄位旨在用於儲存機器的 IEEE 802 地址，該地址用於生成此 UUID 以保證空間唯一性。
     * <p>
     * 節點值僅在基於時間的 UUID（其 version 型別為 1）中才有意義。<br>
     * 如果此 UUID 不是基於時間的 UUID，則此方法丟擲 UnsupportedOperationException。
     *
     ...
     * 將此物件與指定物件比較。
     * <p>
     * 當且僅當引數不為 {@code null}、而是一個 UUID 物件、具有與此 UUID 相同的 varriant、包含相同的值（每一位均相同）時，結果才為 {@code true}。
     *
     * @param obj 要與之比較的物件
     *
